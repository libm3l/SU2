 ...
 */
		if( (SFounds = m3l_Locate(Gnode, "/CFD_2_SIM/ForcesMoments", "/*/*",  (lmchar_t *)NULL)) != NULL){
			...
			if( m3l_get_Found_number(SFounds) != 1)
				Error("socket_SU2out: More then one DX data set found");
/* 
 ...
 */
			if( (FoundNode = m3l_get_Found_node(SFounds, 0)) == NULL)
				Error("socket_SU2out: Did not find 1st data pointer");
 ...
 */		
			if( (x = (lmdouble_t *)m3l_get_data_pointer(FoundNode)) == NULL)
				Error("socket_SU2out: Did not find DX data pointer");
/* 
 ...
 */
		...
		{
			Error("socket_SU2out: P not found\n");
		}
/*
 * locate time
 */	
...
/*
 * NACA 0012 CT1 case, or 2D cases the pithing is usually specified by rool angle
 */
		psi = -2.41*sin(*time*2*3.1415926*5.3398608);  
		theta = 0.; 
		phi = 0.;
